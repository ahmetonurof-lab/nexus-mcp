...
## Sonnet Direktifi: 7 Operasyonel-Hata Reset Noktası (2026-08-12)
- **Direktif:** IDLE'a dönen 17 reset noktasından 7'si 3 gruba ayrıldı (Sonnet baş mühendis review'u). Öncesinde Sonnet'in "lock_bias/BIAS_LOCKED koddan sıfır eşleşme, hiç yazılmamış" iddiası çürütüldü: 27 eşleşme / 5 dosya (bot.py:2, retrace_state.py:17, console_reporter.py:3, exit_lifecycle.py:4, signal_engine.py:1), commit `987f456` kanıt. Sonnet muhtemelen yanlış dizinde (backtest-sniper/src — orada yerel retrace_state.py yok, live'den sys.path import).
- **Grup 1 — full reset korundu (dokunulmadı):** bot.py:680 aktif trade, :808 Zehirli Bölge (cbdr_mult=0.0), :831 qty≤0/bakiye — hesap/oturum seviyesi, FVG değişmez.
- **Grup 2 — `lock_bias()` (`bf89a2e`):** bot.py:756 risk doğrulama (SL mesafesi), :784 eps guard (SL/TP epsilon) — o spesifik FVG'nin geometrisine özgü; bias kilitli kalır, sonraki taze FVG denenir.
- **Grup 3 — `on_operational_fail()` + ardışık sayaç (`a723ab1`):** bot.py:885 order hatası, :907 geçersiz fill → RSM'e `_fail_count` sayacı eklendi (`MAX_CONSECUTIVE_OP_FAILS=3`, stale-event `_stale_n>=3` pattern'iyle aynı); 1-2. hata → lock_bias, **3. ardışık hata → full reset (IDLE)**; `reset()` ve basarili entry (bot.py:1077 `clear_fail_streak()`) sayacı sıfırlar.
- **Testler:** `TestOperationalFail` (5 senaryo — 3. ardişik hatada IDLE zorunlu), bot seviyesinde 7 noktanın her biri için ayrı senaryo: risk_reject_locks_bias, pre_entry eps (BIAS_LOCKED), ENA/SEI/ENA-long FVG clearance (BIAS_LOCKED — Grup 2 tamamlayıcısı), fill_error_locks_bias, three_consecutive_order_fails_full_reset, active_trade_skip_full_reset, toxic_zone_full_reset, skips_when_qty_zero. test_bot.py: **13 fail (bayat: mark_trade_closed/_stage/MIN_FVG_SIZE kayıp — dokunulmadı) + 39 pass**; test_retrace_state.py: **46/46 pass**.
- **Pre-existing fail doğrulaması:** stash + HEAD'de koşarak integration_v2/lifecycle/initial_protection 9 fail'in de değişikliklerden BAĞIMSIZ olduğu kanıtlandı (aynı set HEAD'de).
- **HEAD notu (Reis kuralı):** canlı repo HEAD `a723ab1`, backtest repo `b16d751`. Push: 2 commit (bf89a2e, a723ab1) beklemede.
- **Kalan (düşük öncelik):** backtest analyzer_v5 3-4 saatlik koşu — `fvg_close_confirmed` (fvg.py:35-57) tüm listeyi her çağrıda tarıyor (index erken-çıkış yok) + `_latest_choch`/`_pick_overlap_fvg` (E-varyant kalıntısı) şüpheli; fix taslağı hazır, profil doğrulaması bekliyor.
