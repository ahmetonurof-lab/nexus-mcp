...
STRONG_CONFIDENCE_THRESHOLD: float = getattr(
    config, "STRONG_CONFIDENCE_THRESHOLD", 0.75
)
...
def detect_market_regime(
    ...
    if adx >= 30:
        ...
        if len(bars) >= 14:
            ...
            older_atr = (
                sum(atr_series[-14:-1]) / 13 if len(atr_series) >= 14 else recent_atr
            )
...
def _get_choch_score_for_direction(
    ...
    if not matching:
        if any(c.direction != fvg_direction for c in chochs):
            logger.debug(
                "[SCORING] CHoCH yön uyuşmazlığı: FVG=%s, zıt CHoCH mevcut.",
                fvg_direction,
            )
        return 0.0, ""
...
def analyze_confluence(
    ctx: ScoringContext, fvg_direction: str
) -> tuple[int, list[str]]:
    ...
    # 6. Premium/Discount
    if is_premium_discount_valid(
        ctx.bars, ctx.current_price, fvg_direction, lookback=50
    ):
...
# ─────────────────────────────────────────────────────────
def compute_entry_exit_zones(
    fvg: FVG, atr: float, current_price: float, direction: str
) -> dict[str, float]:
...
def evaluate_trade_signal(
    ...
    directions_to_check = (
        ["bullish", "bearish"] if fvg_direction is None else [fvg_direction]
    )
    ...
    for direction in directions_to_check:
        ...
        d, f, s, r, _ = compute_fvg_component_scores(
            fvg, ctx.bars, ctx.atr, ctx.atr_series, ctx.current_price
        )
        ...
        regime = detect_market_regime(
            ctx.bars, ctx.adx, ctx.ema100, ctx.ema200, ctx.current_price
        )

        # 7. Rejim & Konfluens bazlı kalibrasyon
        base_confidence = fvg_quality.score
        confluence_bonus = min(
            max(0, confluence_count - 1) * CONFLUENCE_WEIGHT, MAX_CONFLUENCE_BONUS
        )
    ...
    try:
        if best_signal is not None:
            ...
            if (
                getattr(ctx, "atr", 0.0)
                and getattr(ctx, "h1_liquidity_level", None) is not None
            ):
            ...
            if getattr(ctx, "atr", 0.0):
                c_span = _clip01(
                    size
                    / max(
                        1e-12, float(ctx.atr) * float(getattr(_cfg, "RANK_SPAN_K", 1.0))
                    )
                )
            else:
                c_span = 0.5
...
def evaluate_all_signals(
    ...
    return {
        "bullish": evaluate_trade_signal(
            ctx, fvg_direction="bullish", min_confidence=min_confidence, vp=vp
        ),
        "bearish": evaluate_trade_signal(
            ctx, fvg_direction="bearish", min_confidence=min_confidence, vp=vp
        ),
    }
...
def generate_market_summary(ctx: ScoringContext) -> dict[str, str | float]:
    ...
    recent_chochs = sum(
        1 for c in ctx.chochs if ctx.bars and (ctx.bars[-1].index - c.bar_index) <= 50
    )
