    ...
    async def get(self, symbol: str) -> list[Bar]:
        ...
        async with self._lock:
            if (
                ...
                or now - self._last_update.get(symbol, 0) > 86400
            ):
                try:
    ...
    async def _fetch(self, symbol: str) -> None:
        try:
            ...
            ohlcv = await loop.run_in_executor(
                None,
                lambda: _http_client.get_klines(
                    symbol, interval="1d", limit=config.D1_BARS, max_retries=2
                ),
            )
            bars = [
                Bar(
            ...
            log.info("D1 cache yenilendi: %s (%d bar)", symbol, len(bars))
        except Exception as e:
            log.error("D1 verisi alınamadı %s: %s", symbol, e)
