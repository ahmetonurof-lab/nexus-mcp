...
class VirtualExchange:
        ...
        for sym, pos in self.positions.items():
            if pos["size"] != 0:
                ...
                if pos["direction"] == "LONG":
                    total += pos["size"] * (
                        pos.get("mark_price", pos["entry_price"]) - pos["entry_price"]
                    )
                else:
                    total += pos["size"] * (
                        pos["entry_price"] - pos.get("mark_price", pos["entry_price"])
                    )
        return total
    ...
    def open_position(
        ...
        logger.info(
            "[BACKTEST] OPEN %s %s %.2f @ %.5f fee=%.4f",
            symbol,
            direction,
            position_size,
            entry_price,
    ...
    def close_position(
        ...
        logger.info(
            "[BACKTEST] CLOSE %s %s pnl=%.2f reason=%s",
            symbol,
            pos["direction"],
            pnl,
            reason,
...
class BacktestEngine:
    ...
    def _feed_bars(self, rows: list[dict], timeframe: str) -> None:
        ...
        if timeframe in ("1d", "1D", "D1", "daily"):
            ...
            logger.warning(
                "Bilinmeyen timeframe: %s — %d bar yüklendi", timeframe, len(bars)
            )
        ...
        for i in range(len(self._bars_m1)):
            ...
            if i % 15 == 0 and self._bars_15m:
                ...
                try:
                    ...
                    self._state_machine.check_poi_retrace(
                        self.symbol, current_15m_bars[-1]
                    )
            ...
            if self.symbol in self._virtual_exchange.positions:
                ...
                if direction == "LONG":
                    if bar_m1.low <= entry - sl_distance:
                        self._virtual_exchange.close_position(
                            self.symbol, entry - sl_distance, "SL"
                        )
                    elif bar_m1.high >= entry + tp_distance:
                        self._virtual_exchange.close_position(
                            self.symbol, entry + tp_distance, "TP"
                        )
                else:  # SHORT
    ...
    def _compute_metrics(self) -> BacktestMetrics:
        ...
        metrics = BacktestMetrics(
            ...
            total_pnl=total_pnl,
            total_pnl_pct=(total_pnl / exchange.initial_balance) * 100
            if exchange.initial_balance
            else 0,
...
def quick_backtest(
    ...
    if data_path.endswith(".csv"):
        ...
        raise ValueError(
            f"Desteklenmeyen dosya formatı: {data_path} (csv veya json olmalı)"
        )
