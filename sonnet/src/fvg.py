    ...
    for i in range(1, len(segment) - 1):
        ...
        if gap_bull > 0:
            ...
            if fvg.size >= min_fvg_size:
                ...
                logger.debug(
                    "[FVG-SIZE] bullish size=%.6f < min=%.6f → atlanıyor.",
                    fvg.size,
                    min_fvg_size,
                )
            ...
            if fvg.size >= min_fvg_size:
                ...
                logger.debug(
                    "[FVG-SIZE] bearish size=%.6f < min=%.6f → atlanıyor.",
                    fvg.size,
                    min_fvg_size,
                )
        ...
        for abs_i in range(scan_from_abs, last_abs + 1):
            ...
            if fvg.direction == "bullish":
                ...
                if b.close < fvg.bottom:
                    ...
                    logger.debug(
                        "[FVG-STATE] bullish invalidated (close=%.5f < bottom=%.5f)",
                        b.close,
                        fvg.bottom,
                    )
                    break
                ...
                if b.close > fvg.top:
                    ...
                    object.__setattr__(fvg, "filled", False)
                    logger.debug(
                        "[FVG-STATE] bearish invalidated (close=%.5f > top=%.5f)",
                        b.close,
                        fvg.top,
                    )
                    break
...
def score_sweep(bars: list[Bar], fvg: FVG, lookback: int = 5) -> float:
    ...
    if fvg.direction == "bullish":
        ...
        sweep_count = sum(
            1 for j in range(window_start, fvg_pos) if bars[j].low < swing_low
        )
    else:
        swing_high = max(bars[j].high for j in range(window_start, fvg_pos))
        sweep_count = sum(
            1 for j in range(window_start, fvg_pos) if bars[j].high > swing_high
        )
...
def compute_fvg_quality(
    ...
    return FVGQuality(
        displacement=d, fvg_size=f, sweep=s, retest=r, score=min(score, 1.0)
    )
