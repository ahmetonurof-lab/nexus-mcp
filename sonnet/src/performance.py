...
def record_trade(trade: dict) -> None:
    ...
    try:
        ...
        log.info(
            f"[PERF] {symbol} kaydedildi | pnl={pnl:.2f} win_rate={get_win_rate(symbol):.1f}%"
        )
...
def _build_stat(symbol: str) -> dict:
    ...
    return {
        ...
        "avg_adx": (
            round(s["adx_sum"] / s["adx_count"], 1) if s["adx_count"] > 0 else 0.0
        ),
...
def _write_summary_csv(trade: dict) -> None:
    ...
    try:
        ...
        if isinstance(entry_raw, int | float) and entry_raw > 1_000_000_000:
            entry_time = datetime.fromtimestamp(entry_raw / 1000, tz=UTC).strftime(
                "%Y-%m-%d %H:%M:%S"
            )
        else:
        ...
        if isinstance(exit_raw, int | float) and exit_raw > 1_000_000_000:
            close_time = datetime.fromtimestamp(exit_raw / 1000, tz=UTC).strftime(
                "%Y-%m-%d %H:%M:%S"
            )
        else:
...
def _write_strategy_csv(trade: dict) -> None:
    try:
        ...
        fvg_case = (
            "C" if trade.get("fvg_missed") else ("A" if trade.get("retrace") else "")
        )
        fvg_ce = ""
        ...
        if isinstance(exit_raw, int | float) and exit_raw > 1_000_000_000:
            exit_time = datetime.fromtimestamp(exit_raw / 1000, tz=UTC).strftime(
                "%Y-%m-%d %H:%M:%S"
            )
        else:
