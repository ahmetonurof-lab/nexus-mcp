...
# ─────────────────────────────────────────────────────────
CHoCH_SFP_FOLLOWTHROUGH: Final[int] = (
    2  # kaç bar boyunca level ötesinde kapanış gerekir
)
...
class CHoCH:
    ...
    timeframe: str = "5m"
    strength: float = (
        0.0  # CHoCH kalite gücü [0.0, 1.0] — penetration + SFP follow-through
    )
...
class AnalysisResult:
    ...
    ema200: float = 0.0
    close_d1: float = 0.0
    vp_levels: object | None = (
        None  # VPLevels (volume_profile.py) — TYPE_CHECKING ile import
    )
