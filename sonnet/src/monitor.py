...
def get_prometheus_metrics() -> str:
    ...
    lines.append(
        _prometheus_metric(
            "nexus_total_fills", "Total fills across all symbols", _COUNTER, total_fills
        )
    )
    lines.append(
        _prometheus_metric(
            "nexus_total_rejects",
            "Total rejects across all symbols",
            _COUNTER,
            total_rejects,
        )
    )
    ...
    return {
        ...
        "panels": [
            ...
            },
            {
                ...
                "targets": [
                    {
                        "expr": 'rate(nexus_signal_count_total{symbol=~"$symbol"}[5m])',
                        "legendFormat": "signals",
                    },
                    {
                        "expr": 'rate(nexus_order_count_total{symbol=~"$symbol"}[5m])',
                        "legendFormat": "orders",
                    },
                    {
                        "expr": 'rate(nexus_fill_count_total{symbol=~"$symbol"}[5m])',
                        "legendFormat": "fills",
                    },
                ],
            },
            {
                ...
                "targets": [
                    {
                        "expr": 'nexus_tick_seconds{symbol=~"$symbol"}',
                        "legendFormat": "{{symbol}}",
                    },
                ],
            },
