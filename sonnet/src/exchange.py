...
class BinanceHTTPClient:
    ...
    def _request(
        ...
        raise (
            last_error
            if last_error
            else Exception(f"{method} {path}: max_retries exhausted")
        )
    ...
    def _load_exchange_info(self, force: bool = False) -> dict:
        ...
        with self._info_lock:
            if (
                not force
                and self._exchange_info
                and (now - self._exchange_info_ts) < 300
            ):
    ...
    def create_order(
        ...
        log.info(
            "[ORDER-DEBUG] %s %s %s req_params=%s", symbol, side, order_type, req_params
        )
        ...
        if self.portfolio_margin and result.get("status") == "ok":
            ...
            try:
                ...
                log.exception(
                    "[ORDER_PM_EXC] %s get_open_orders başarısız: %s", symbol, e
                )
        ...
        if not result.get("orderId") and not result.get("id"):
            ...
            def _match_order(o: dict) -> bool:
                return (
                    o.get("symbol") == symbol
                    and (o.get("type") or o.get("orderType") or "")
                    == order_type.upper()
            ...
            try:
                orders = self.get_open_orders(symbol)
                log.info(
                    "[ORDER_FALLBACK_DEBUG] method=get_open_orders symbol=%s orders=%s",
                    symbol,
                    orders,
                )
            ...
            if self.portfolio_margin:
                try:
                    ...
                    log.info(
                        "[ORDER_FALLBACK_DEBUG] method=std_openOrders symbol=%s orders=%s",
                        symbol,
                        orders,
                    )
                    if isinstance(orders, list) and orders:
            ...
            log.warning(
                "[ORDER_FALLBACK_EMPTY] %s için hiçbir GET kaynağında eşleşen emir bulunamadı",
                symbol,
            )
    ...
    def create_algo_order(
        ...
        if params:
            ...
            cleaned_params = {
                ...
                if k not in ["reduceOnly", "closePosition"]
            }
        ...
        if (
            not result.get("algoId")
            and not result.get("orderId")
            and not result.get("id")
        ):
            import time as _time
            ...
            try:
                ...
                log.info(
                    "[ALGO_FALLBACK_DEBUG] method=get_algo_orders symbol=%s orders=%s",
                    symbol,
                    orders,
                )
                if isinstance(orders, list) and orders:
            ...
            if self.portfolio_margin:
                try:
                    ...
                    log.info(
                        "[ALGO_FALLBACK_DEBUG] method=std_openAlgoOrders symbol=%s orders=%s",
                        symbol,
                        orders,
                    )
                    if isinstance(orders, list) and orders:
            ...
            try:
                orders = self.get_open_orders(symbol)
                log.info(
                    "[ALGO_FALLBACK_DEBUG] method=get_open_orders symbol=%s orders=%s",
                    symbol,
                    orders,
                )
                if isinstance(orders, list) and orders:
            ...
            log.warning(
                "[ALGO_FALLBACK_EMPTY] %s için hiçbir GET kaynağında eşleşen emir bulunamadı",
                symbol,
            )
    ...
    def create_stop_order_standard(
        ...
        if ep is None:
            log.warning(
                "[STOP_ORDER_STANDARD] Portfolio Margin'de bu endpoint desteklenmiyor"
            )
            return {}
    ...
    def cancel_order(
        self, symbol: str, order_id: str, is_algo: bool = False
    ) -> dict[str, Any]:
    ...
    def get_positions(self, symbol: str | None = None) -> list[dict[str, Any]]:
        ...
        return self._request(
            "GET", self._ep("/fapi/v2/positionRisk"), params, signed=True
        )
    ...
    def set_margin_mode(self, symbol: str, margin_type: str = "ISOLATED") -> dict:
        ...
        if ep is None:
            log.info(
                "[MARGIN-MODE] Portfolio Margin modunda marginType ayarı desteklenmiyor — atlanıyor."
            )
            return {}
        return self._request(
            "POST",
    ...
    def get_open_orders(self, symbol: str) -> list[dict]:
        ...
        result = self._request(
            "GET", self._ep("/fapi/v1/openOrders"), {"symbol": symbol}, signed=True
        )
    ...
    def get_algo_orders(self, symbol: str) -> list[dict]:
        ...
        result = self._request(
            "GET", self._ep("/fapi/v1/openAlgoOrders"), {"symbol": symbol}, signed=True
        )
        if isinstance(result, list):
            return result
        if isinstance(result, dict):
