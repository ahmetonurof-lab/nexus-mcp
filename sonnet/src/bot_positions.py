...
class PositionManager:
            ...
            async with prefill_sem:
                try:
                    ...
                    ohlcv = await loop.run_in_executor(
                        None,
                        lambda: self.http_client.get_klines(
                            s, interval=t, limit=limit, max_retries=2
                        ),
                    )
                    bars = [
                        Bar(
    ...
    async def startup_cleanup(self) -> None:
        ...
        try:
            ...
            if not positions_list:
                log.warning(
                    "🧹 CLEANUP | positions_list BOŞ — hiçbir emir silinmeyecek"
                )
                return
            ...
            missing_symbols = [
                s for s in self.active_trades if s not in symbols_with_position
            ]
            if missing_symbols:
                log.warning(
                ...
                retry_pos = await loop.run_in_executor(
                    None, lambda: self.http_client.get_positions()
                )
            ...
            if not symbols_with_position and self.active_trades:
                log.warning(
                    "🧹 CLEANUP | API'de pozisyon yok ama local'de %d trade var — ATLANIYOR",
                    len(self.active_trades),
                )
                return
            if not self.active_trades and symbols_with_position:
                log.warning(
                    "🧹 CLEANUP | Local state boş ama API'de %d pozisyon var — ATLANIYOR",
                    len(symbols_with_position),
                )
                return
            if not symbols_with_position and not self.active_trades:
                log.warning(
                    "🧹 CLEANUP | API'de pozisyon YOK ve local state BOŞ — ATLANIYOR"
                )
                return
            ...
            all_orders: list = (
                all_orders_raw if isinstance(all_orders_raw, list) else []
            )
            try:
                ...
                try:
                    if symbol not in symbols_with_position:
                        if symbol in self.active_trades:
                            log.warning(
                                "🧹 [ORPHAN-GUARD] %s API'de pozisyon yok ama local'de trade var — ATLANIYOR",
                                symbol,
                            )
                            continue
                        log.warning(
                            "🧹 [ORPHAN] %s | %d emir var ama POZİSYON YOK → iptal",
                            symbol,
                            len(orders),
                        )
                        for o in orders:
                            ...
                            if order_id:
                                await self.rest.cancel_order(
                                    order_id, symbol, reason="orphan", is_algo=is_algo
                                )
                                total_cancelled += 1
                        ...
                        if len(sl_orders) > 1 or len(tp_orders) > 1:
                            ...
                            if len(sl_orders) > 1:
                                sl_orders.sort(
                                    key=lambda o: self.rest.get_order_timestamp(o),
                                    reverse=True,
                                )
            ...
            if total_cancelled:
                log.warning(
                    "🧹 STARTUP CLEANUP | TOPLAM %d EMİR İPTAL EDİLDİ", total_cancelled
                )
            else:
        ...
        try:
            ...
            for pos in positions:
                ...
                for attempt in range(3):
                    ...
                    try:
                        algo_raw = await self.rest.get(
                            "/fapi/v1/openAlgoOrders", f"symbol={symbol}"
                        )
            ...
            if self.active_trades:
                log.info(
                    "[RECOVER] %d pozisyon envantere alındı", len(self.active_trades)
                )
            else:
    ...
    async def sync(self, current_bar: Bar) -> None:
        ...
        try:
            ...
            if not positions:
                log.warning(
                    "[SYNC-POSITIONS] pozisyon listesi boş — trade'ler korunuyor"
                )
                return
            ...
            for symbol, trade in list(self.active_trades.items()):
                ...
                if n_sl > 1 or n_tp > 1:
                    ...
                    if n_sl > 1:
                        sl_orders.sort(
                            key=lambda o: self.rest.get_order_timestamp(o), reverse=True
                        )
                        for o in sl_orders[1:]:
                            order_id = o.get("algoId") or o.get("orderId")
                            if order_id:
                                try:
                                    ...
                                    log.warning(
                                        "🛡️ [INFAZ-SL] %s | orderId=%s iptal BAŞARISIZ: %s",
                                        symbol,
                                        order_id,
                                        cancel_err,
                                    )
                        ...
                        trade["sl_order_id"] = str(
                            sl_orders[0].get("algoId")
                            or sl_orders[0].get("orderId")
                            or ""
                        )
                        trade["current_sl"] = self.rest.get_order_price(
                            sl_orders[0]
                        ) or trade.get("current_sl", 0)
                    if n_tp > 1:
                        tp_orders.sort(
                            key=lambda o: self.rest.get_order_timestamp(o), reverse=True
                        )
                        for o in tp_orders[1:]:
                            ...
                            if order_id:
                                try:
                                    ...
                                    log.warning(
                                        "🛡️ [INFAZ-TP] %s | orderId=%s iptal BAŞARISIZ: %s",
                                        symbol,
                                        order_id,
                                        cancel_err,
                                    )
                        ...
                        trade["tp_order_id"] = str(
                            tp_orders[0].get("algoId")
                            or tp_orders[0].get("orderId")
                            or ""
                        )
                        trade["tp"] = self.rest.get_order_price(
                            tp_orders[0]
                        ) or trade.get("tp", 0)
                    ...
                    if n_sl_now == 0 or n_tp_now == 0:
                        ...
                        try:
                            await self.repair_protection(
                                symbol, trade, n_sl_now > 0, n_tp_now > 0
                            )
                        except Exception as e:
                            log.critical(
                    ...
                    trade["tp"] = self.rest.get_order_price(tp_orders[0]) or trade.get(
                        "tp", 0
                    )
                    ...
                    log.warning(
                        "⚠️ MISSING PROTECTION | %s | SL=%d TP=%d → Safe Mode",
                        symbol,
                        n_sl,
                        n_tp,
                    )
                    ...
                    try:
                        ...
                        log.critical(
                            "🚨 [SYNC] %s protection/repair KRİTİK HATA: %s", symbol, e
                        )
                    finally:
                        trade["protection_repairing"] = False
            ...
            for symbol, trade in list(self.active_trades.items()):
                if symbol not in exchange_positions:
                    ...
                    if tp_price:
                        if direction == "long":
                            close_reason = (
                                "TP" if exit_price >= tp_price * 0.995 else "SL"
                            )
                        else:
                            close_reason = (
                                "TP" if exit_price <= tp_price * 1.005 else "SL"
                            )
                    else:
                        close_reason = "closed"
                    ...
                    try:
                        ...
                        log.warning(
                            "[SYNC] %s cancel_all_orders hatası: %s", symbol, cancel_err
                        )

                    self.clear_state(symbol)
                    self.executor.reset_cooldown(symbol)
                    log.info(
                        "EXCHANGE SYNC: %s kapandı | 🔴 CIKIS=%.4f pnl=%.2f USDT",
                        symbol,
                        exit_price,
                        pnl,
                    )
    ...
    async def repair_protection(
        self, symbol: str, trade: dict, has_sl: bool, has_tp: bool
    ) -> None:
        ...
        try:
            ...
            if not has_tp and trade.get("tp"):
                ...
                if (direction == "long" and mark_price >= tp_price) or (
                    ...
                    log.critical(
                        "🚘 [SORGUSUZ İNFAZ] %s TP (%.5f) zaten geçildi — MARKET kapatılıyor!",
                        symbol,
                        tp_price,
                    )
                    await self.executor.close_position(
                        symbol, reason="tp_already_hit_repair"
                    )
                    return
            ...
            if sl_ok and tp_ok:
                ...
                log.warning(
                    "⚠️ [REPAIR] %s doğrulama başarısız SL_ok=%s TP_ok=%s",
                    symbol,
                    sl_ok,
                    tp_ok,
                )
    ...
    async def create_protection(self, symbol: str, trade: dict) -> None:
        ...
        try:
            ...
            mark_price = float(pos.get("markPrice", 0)) or trade.get(
                "last_price", entry
            )
            tier = risk_mgr._tier(symbol)
            buf = tier["sl_buffer"]
            ...
            if direction == "long":
                ...
                if mark_price >= tp_candidate:
                    log.critical(
                        "🚘 [SORGUSUZ İNFAZ] %s TP zaten geçildi — MARKET kapatılıyor!",
                        symbol,
                    )
                ...
                if mark_price <= sl_candidate:
                    log.critical(
                        "🚨 [CREATE] %s SL zaten geçildi — EMERGENCY kapatılıyor!",
                        symbol,
                    )
            ...
            sl_id = str(
                (sl_resp or {}).get("algoId")
                or (sl_resp or {}).get("orderId")
                or (sl_resp or {}).get("id")
                or ""
            )
            ...
            if tp is not None:
                try:
                    ...
                    if "-2021" in err_str:
                        log.warning(
                            "🟡 [CREATE] %s TP hemen tetiklenirdi — atlanıyor", symbol
                        )
                    elif "-4130" in err_str:
                        log.warning("🟡 [CREATE] %s TP/SL zaten mevcut", symbol)
                    else:
                        raise
    ...
    async def update_sl_order(self, symbol: str, trade: dict, new_sl: float) -> None:
        ...
        try:
            ...
            if not old_sl:
                ...
                log.info(
                    "🛡️ SL UPDATE | %s | yeni SL=%.8f (id=%s)", symbol, new_sl, new_id
                )
                return
            ...
            if "algoId" in old_sl:
                ...
                await self.rest.cancel_order(
                    old_id, symbol, reason="sl_update", is_algo=True
                )
                await asyncio.sleep(0.2)
                sl_side = "sell" if trade["direction"] == "long" else "buy"
                sl_resp = await self.executor.client.create_stop_order(
            ...
            new_id = str(
                result.get("algoId") or result.get("orderId") or result.get("id") or ""
            )
            if new_id:
                trade["sl_order_id"] = new_id
            log.info(
            ...
            try:
                ...
                log.critical(
                    "🚨 SL FALLBACK BAŞARISIZ | %s | EMERGENCY CLOSE: %s",
                    symbol,
                    fallback_err,
                )
                try:
                    await self.executor.close_position(
                        symbol, reason="emergency_sl_update_fail"
                    )
                    log.critical("🚨 EMERGENCY CLOSE BAŞARILI | %s", symbol)
                except Exception as close_err:
                    log.critical(
                        "🚨 EMERGENCY CLOSE BAŞARISIZ | %s | manuel müdahale! hata=%s",
                        symbol,
                        close_err,
                    )
    ...
    async def safe_manage_open_trades(self, current_bar: Bar) -> None:
        ...
        try:
            ...
            log.critical(
                "[MANAGE-SAFE] manage_open_trades hatası: %s", str(e), exc_info=True
            )
    ...
    async def manage_open_trades(self, current_bar: Bar) -> None:
        ...
        for symbol, trade in list(self.active_trades.items()):
            try:
                ...
                if not pos or abs(float(pos.get("contracts", 0))) == 0:
                    log.warning(
                        "[MANAGE-RACE] %s pozisyon API'de bulunamadı — ATLANIYOR",
                        symbol,
                    )
                    continue
            except Exception as e:
                log.warning(
                    "[MANAGE-RACE] %s pozisyon sorgusu başarısız: %s", symbol, e
                )
                continue

            if trade.get("protection_missing"):
                log.warning("🟡 SAFE MODE | %s | sadece izleme", symbol)
                continue
            if trade.get("protection_repairing"):
                log.warning("🟡 REPAIR MODE | %s | sadece izleme", symbol)
                continue
            ...
            if open_time and (current_time_ms - open_time) < 300_000:
                ...
                log.info(
                    "[MANAGE] %s henüz taze (kalan: %dsn) — Breakeven/Trailing atlandı",
                    symbol,
                    remaining,
                )
                continue
            ...
            try:
                ...
                symbol_close = symbol_bars[-1].close if symbol_bars else None
                current_price = (
                    trade.get("last_price") or symbol_close or trade.get("entry", 0)
                )
                sl_current = trade.get("current_sl", trade["initial_sl"])

                if not trade.get(
                    "breakeven_done", False
                ) and risk_mgr.should_move_to_breakeven(trade, current_price):
                    ...
                    if config.BREAKEVEN_LOG_ENABLED:
                        ...
                        log.info(
                            "[BE] %s breakeven | yeni SL=%.8f | d1_adx=%.1f (%s)",
                            symbol,
                            new_sl,
                            d1_adx,
                            adx_flag,
                    ...
                    if (
                        config.BREAKEVEN_LOG_ENABLED
                        and current_time_ms - self._last_be_summary > 1_800_000
                    ):
                        self._last_be_summary = current_time_ms
                        total_be = sum(v["count"] for v in self._breakeven_log.values())
                        total_adx35 = sum(
                            v["adx_gt_35"] for v in self._breakeven_log.values()
                        )
                        corr_pct = (
                            (total_adx35 / total_be * 100) if total_be > 0 else 0.0
                        )
                        log.info(
                            "[BE-SUMMARY] toplam=%d | ADX>35'te=%d (%.1f%%)",
                            total_be,
                            total_adx35,
                            corr_pct,
                        )
                    ...
                    if new_sl != sl_current:
                        trade["current_sl"] = new_sl
                        log.info(
                            "[TRAIL] %s SL: %.8f → %.8f", symbol, sl_current, new_sl
                        )
                        await self.update_sl_order(symbol, trade, new_sl)
