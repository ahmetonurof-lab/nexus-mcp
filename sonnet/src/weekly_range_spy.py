...
def check_5m(symbol: str, bars_d1: list[Bar], current_5m_bar: Bar) -> None:
    ...
    if st["last_week_key"] != current_week_key:
        ...
        logger.debug(
            "[WEEKLY-SPY] %s yeni hafta — state sıfırlandı | HH=%.5f LL=%.5f",
            symbol,
            hh,
            ll,
        )
...
def log_sweep(
    symbol: str, side: Literal["HH", "LL"], level: float, close: float
) -> None:
...
def _compute_weekly_range(bars_d1: list[Bar]) -> tuple[float, float] | None:
    ...
    start_dt = datetime(
        last_monday.year, last_monday.month, last_monday.day, tzinfo=UTC
    )
    end_dt = datetime(
        last_sunday.year,
        last_sunday.month,
