...
class ExchangeClient:
    ...
    def _validate_min_amount(
        self, symbol: str, amount: float, original_amount: float | None = None
    ) -> bool:
    ...
    async def create_order(
        ...
        if amount is not None and amount > 0:
            ...
            if not self._validate_min_amount(symbol, amount, raw_amount):
                raise ValueError(
                    f"{symbol} amount={amount} < minimum precision requirement"
                )
        ...
        if params and "stopPrice" in params and params["stopPrice"] is not None:
            params = dict(params)
            params["stopPrice"] = self._apply_price_precision(
                symbol, float(params["stopPrice"])
            )
        ...
        log.info(
            "[ORDER-DEBUG] trader.py create_order params=%s amount=%s price=%s",
            params,
            amount,
            price,
        )
    ...
    async def fetch_position(self, symbol: str) -> dict | None:
        ...
        try:
            positions = await loop.run_in_executor(
                None, lambda: self.http.get_positions(symbol)
            )
    ...
    async def cancel_order(self, order: dict, symbol: str) -> bool:
        ...
        try:
            ...
            if "-2011" in err_str or "Unknown order" in err_str:
                log.info(
                    "cancel_order: %s order_id=%s zaten yok (ok)", symbol, order_id
                )
                return True
    ...
    async def get_open_orders(self, symbol: str) -> list[dict]:
        ...
        loop = asyncio.get_running_loop()
        return await loop.run_in_executor(
            None, lambda: self.http.get_all_open_orders(symbol)
        )
    ...
    async def cancel_all_orders(self, symbol: str) -> int:
        ...
        log.info(
            "[CANCEL_ALL] %s → %d/%d emir iptal edildi", symbol, cancelled, len(orders)
        )
        return cancelled
...
class LiveExecutor:
    ...
    async def _wait_for_fill(self, symbol: str, timeout: float = 5.0):
        ...
        log.warning(
            "[FILL] %s pozisyon onaylanamadı (%.0fs timeout), yine de devam",
            symbol,
            timeout,
        )
    ...
    async def send_order(
        ...
        if hasattr(trade_params, "symbol"):
            ...
            sl_price = (
                ...
                else getattr(trade_params, "sl", None)
            )
            tp_price = (
                take_profit
                if take_profit is not None
                else getattr(trade_params, "tp", None)
            )
        else:
            ...
            tp_price = (
                take_profit if take_profit is not None else trade_params.get("tp")
            )
        ...
        async with lock:
            ...
            if existing is not None:
                log.warning(
                    "send_order: %s için zaten açık pozisyon var → emir atlanıyor",
                    symbol,
                )
            ...
            try:
                ...
                if entry_order_type.upper() == "STOP_MARKET":
                    trigger = (
                        trade_params.entry if hasattr(trade_params, "entry") else 0.0
                    )
                    if current_price and trigger > 0:
                        if direction.lower() == "long":
                            trigger = max(
                                trigger,
                                current_price * (1.0 + max(0.0, stop_offset_pct)),
                            )
                        else:
                            trigger = min(
                                trigger,
                    ...
                    try:
                        stop_resp = await self.client.create_order(
                            ...
                            amount=lot,
                            price=None,
                            stop_price=self.client._apply_price_precision(
                                symbol, trigger
                            ),
                            params={
                                "newClientOrderId": f"{client_order_id}_entry_stop"
                            },
                        )
                        order_id = str(
                            stop_resp.get("orderId")
                            or stop_resp.get("clientOrderId")
                            or ""
                        )
                        log.info(
                ...
                if sl_price is not None:
                    ...
                    for sl_attempt in range(2):
                        try:
                            ...
                            sl_resp = await self.client.create_algo_order(
                                ...
                                stop_price=sl_price,
                                params={
                                    "newClientOrderId": f"{sl_order_id_prefix}_{sl_attempt}"
                                },
                            )
                            ...
                            log.info(
                                "SL EMİR ✓: %s stopPrice=%.8f algo_id=%s",
                                symbol,
                                sl_price,
                                sl_order_id,
                            )
                    ...
                    if not sl_success:
                        log.critical(
                            "🚨 EMERGENCY CLOSE | %s | SL yazılamadı (2 deneme başarısız)",
                            symbol,
                        )
                        emergency_ok = await self.close_position(
                            symbol, reason="emergency_sl_fail"
                        )
                        if emergency_ok:
                            log.critical(
                                "🚨 EMERGENCY CLOSE BAŞARILI | %s | pozisyon market kapatıldı",
                                symbol,
                            )
                        else:
                            log.critical(
                                "🚨 EMERGENCY CLOSE BAŞARISIZ | %s | manuel müdahale gerekli!",
                                symbol,
                            )
                        raise RuntimeError(
                            f"SL yazılamadı, EMERGENCY CLOSE tetiklendi: {symbol}"
                        )
                    ...
                    for tp_attempt in range(2):
                        try:
                            ...
                            tp_resp = await self.client.create_algo_order(
                                ...
                                stop_price=tp_price,
                                params={
                                    "newClientOrderId": f"{tp_order_id_prefix}_{tp_attempt}"
                                },
                            )
                            ...
                            log.info(
                                "TP EMİR ✓: %s stopPrice=%.8f algo_id=%s",
                                symbol,
                                tp_price,
                                tp_order_id,
                            )
                ...
                if not tp_success:
                    log.warning(
                        "⚠️ TP YAZILAMADI | %s | pozisyon korumasız kaldı (2 deneme başarısız)",
                        symbol,
                    )
                ...
                if not sl_order_id:
                    log.critical(
                        "🚨 SL ORDER ID ALINAMADI | %s | pozisyon korumasız!", symbol
                    )
                if not tp_order_id:
