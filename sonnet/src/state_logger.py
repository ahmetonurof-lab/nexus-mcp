...
def _today_path() -> str:
    return os.path.join(
        SUMMARY_DIR, f"summary_{datetime.now().strftime('%Y-%m-%d')}.csv"
    )
...
def write_snapshot(symbol: str, state) -> None:
    try:
        ...
        fvg_case = (
            "C"
            if getattr(state, "fvg_missed", False)
            else ("A" if getattr(state, "retrace_seen", False) else "")
        )
        fvg_upper = getattr(state, "fvg_upper", None)
        fvg_lower = getattr(state, "fvg_lower", None)
        fvg_ce = (
            round((fvg_upper + fvg_lower) / 2, 6) if (fvg_upper and fvg_lower) else ""
        )
        sweep_side = ""
