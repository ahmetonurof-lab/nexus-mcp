            ...
            if side_exit == "long":
                if current.low <= trade_exit["sl"]:
                    ...
                    await self._exit_trade(
                        sym, trade_exit, current, trade_exit["exit_timestamp"]
                    )
    ...
    async def _try_entry(
        ...
        if not can_open_trade(sym):
            self._pl(
                sym,
                "quota",
                f"QUOTA: {sym} bugün işlem kotası doldu, sinyal pas geçiliyor",
            )
            ...
            try:
                ...
                if valid_qty <= 0:
                    ...
                    if mkt_id:
                        ...
                        log.warning(
                            "[ORDER] %s MARKET entry BASARISIZ — SL/TP atlandi", sym
                        )
            ...
            try:
                    ...
                    for o in open_orders if isinstance(open_orders, list) else []:
                        ...
                        if oid:
                            ...
                            await self.rest.cancel_order(
                                oid, sym, reason="exit_close", is_algo=is_algo
                            )
                except Exception as e:
                ...
                if valid_qty > 0:
                    close_resp = await self.rest.place_market_order(
                        sym, close_side, valid_qty
                    )
                    if close_resp.get("orderId") or close_resp.get("id"):
                        log.info(
                            "[CLOSE] %s pozisyon kapatildi orderId=%s",
                            sym,
                            close_resp.get("orderId", ""),
                        )
                else:
                    log.warning(
                        "[CLOSE] %s qty=%.8f minQty altinda, pozisyon kapatilamadi",
                        sym,
                        qty,
                    )
            except Exception as e:
