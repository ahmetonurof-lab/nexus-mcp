...
BASE_URL = (
    os.getenv("BASE_URL", "https://demo-fapi.binance.com")
    if TESTNET
        ...
        async def on_account_update(msg: dict) -> None:
            ...
            for bal in balances:
                ...
                if asset in ("USDT", "FDUSD", "USDC"):
                    self.positions._wallet_balance = float(
                        bal.get("wb", self.positions._wallet_balance)
                    )
                    self.positions._available_balance = float(
                        bal.get("bc", self.positions._available_balance)
                    )
                    self.positions._balance = self.positions._available_balance
            if balances:
                log.debug(
                    "[USER_DATA] ACCOUNT_UPDATE | reason=%s | %d balance",
                    reason,
                    len(balances),
                )
            ...
            if positions_data:
                log.debug(
                    "[USER_DATA] ACCOUNT_UPDATE | reason=%s | %d pozisyon",
                    reason,
                    len(positions_data),
                )
            ...
            return web.json_response(
                {
                    ...
                    "live_symbols": sum(
                        1 for v in sym_data if v.get("status") == "LIVE"
                    ),
            ...
            return web.json_response(
                {
                    ...
                    "symbols": {
                        sym: {
                            ...
                            "adx_pct": round(
                                (info["adx_gt_35"] / info["count"]) * 100, 1
                            )
                            if info["count"] > 0
                            else 0.0,
                        }
