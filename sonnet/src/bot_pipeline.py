...
class TradingPipeline:
    ...
    async def on_1m_close(self, symbol: str, bars_m1: list[Bar]) -> None:
        try:
            ...
            await self._emit_events(
                symbol, bars_d1, bars_h4, bars_h1, bars_15m, bars_m1
            )
            self._state_debug_log(symbol)
    ...
    async def _check_partial_entry(
        ...
        try:
            ...
            async with get_lock(symbol):
                if symbol in self.active_trades:
                    return
                await self._send_partial_order(
                    symbol, bars_m1, current_state, scale, pen
                )
    ...
    async def _send_partial_order(
        ...
        self.active_trades[symbol] = self._build_trade_dict(
            tp, current_state, partial=True
        )
    ...
    async def _check_ready_to_enter(
        ...
        async with get_lock(symbol):
            ...
            self.active_trades[symbol] = self._build_trade_dict(
                trade_params, current_state, partial=False
            )
            self.state_machine.set_state(symbol, SetupState.ENTERED)
            self.positions.flush_state()
            log.info(
    ...
    async def _emit_events(
        ...
        if bars_m1 is None or len(bars_m1) < 5:
            log.warning(
                "[SKIP] %s yetersiz 1m bar: %d", symbol, len(bars_m1) if bars_m1 else 0
            )
            return
    ...
    def _state_debug_log(self, symbol: str) -> None:
        """Her 1m callback'te state debug logu."""
        current_state = self.state_machine.get(symbol)
    ...
    def _build_trade_dict(
        self, trade_params: object, current_state: object, partial: bool
    ) -> dict:
