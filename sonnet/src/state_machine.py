...
class StateMachine:
    def __init__(self, config=None):
        ...
        self._last_bar: Bar | None = (
            None  # zombie setup invalidation için son kapanan bar
        )
    ...
    def _handle_mss(self, state: SymbolState, event: dict):
        ...
        if state.state == SetupState.WAIT_CONFIRM and state.direction is not None:
            ...
            state.displacement_origin = event.get("impulse_origin") or event.get(
                "level"
            )
            state.direction = event.get("direction")
            max_wait = (
                getattr(self.config, "MAX_SETUP_WAIT_HOURS", 16.0)
                if self.config
                else 16.0
            )
        ...
        if state.state in (
            ...
            if sweep_tf == "15m":
                max_wait = (
                    getattr(self.config, "MAX_SETUP_WAIT_HOURS_15M", 8.0)
                    if self.config
                    else 8.0
                )
            else:
            ...
            state.state = SetupState.WAIT_RETRACE
            logger.info(
                "[%s] MSS confirmed → WAIT_RETRACE | expires_in=%.0fh",
                state.symbol,
                max_wait,
            )
        else:
    ...
    def _handle_fvg(self, state: SymbolState, event: dict):
        ...
        if state.state in (
            ...
            state.fvg_lower = None
            logger.debug(
                "[%s] FVG event reddedildi — state=%s", state.symbol, state.state
            )
            return
        ...
        if state.state == SetupState.WAIT_NEW_FVG:
            ...
            if not event.get("is_active", True):
                logger.debug(
                    "[%s] WAIT_NEW_FVG: FVG is_active=False — reddedildi", state.symbol
                )
                return
            state.retrace_seen = False
            state.is_ce_tap = False
        ...
        logger.info(
            "[%s] FVG kaydedildi | upper=%.5f lower=%.5f",
            state.symbol,
            state.fvg_upper,
            state.fvg_lower,
        )
    ...
    def check_poi_retrace(
        self, symbol: str, current_bar: Bar, atr: float = 0.0
    ) -> None:
        """
    ...
    def _handle_ltf(self, state: SymbolState, event: dict):
        ...
        if state.fvg_upper is None or state.fvg_lower is None:
            if (
                state.state == SetupState.WAIT_POI_CONFIRM
                and state.poi_anchor is not None
            ):
                pass
            else:
                logger.warning(
                    "[%s] LTF confirm geldi ama FVG seviyeleri yok — atlandı",
                    state.symbol,
                )
                return
        ...
        if state.state == SetupState.WAIT_CONFIRM:
            ...
            engine = PenetrationEngine(
                state.fvg_upper, state.fvg_lower, state.direction
            )
            ...
            logger.info(
                "[%s] LTF confirm → READY_TO_ENTER (Case A) pen=%.2f", state.symbol, pen
            )
        elif state.state == SetupState.WAIT_POI_CONFIRM:
            state.state = SetupState.READY_TO_ENTER
            logger.info(
                "[%s] LTF confirm → READY_TO_ENTER (Case C / poi_anchor)", state.symbol
            )
    ...
    def _check_stale_state(self, state: SymbolState, current_time: datetime) -> bool:
        ...
        if state.state in stale_states:
            if (
                ...
                and current_time.timestamp() > state.expires_at
            ):
                logger.warning(
    ...
    def _evaluate(
        ...
        try:
            ...
            logger.warning(
                "[%s] ADAPTIVE evaluate hatası (sessiz): %s",
                state.symbol,
                e,
                exc_info=True,
            )
