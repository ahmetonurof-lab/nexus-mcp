...
class MarketAnalyzer:
    ...
    def reset_symbol_cache(self) -> None:
        ...
        logger.debug(
            "[CACHE-RESET] %s _emitted_fvg_ids + _seen_mss + _mss_state temizlendi",
            self.symbol,
        )
    ...
    def _sweep_on_bars(
        ...
        strength = (
            ...
            else getattr(config, "SWEEP_SWING_STRENGTH", 2)
        )
        pen_atr_mult = (
            pen_atr_override
            if pen_atr_override is not None
            else getattr(config, "SWEEP_PENETRATION_ATR", 0.10)
        )
    ...
    def _detect_mss_events(
        ...
        if since_bar_index is None:
            logger.debug(
                "[MSS] %s since_bar_index=None → sweep yok, MSS taraması atlandı",
                symbol,
            )
            return []
    ...
    def _find_retracement_swing(
        ...
        for i in range(left, len(post_entry) - right):
            ...
            if direction == "LONG":
                ...
                ) and all(bar.low <= post_entry[i + j].low for j in range(1, right + 1))
                if is_pivot:
                    candidates.append(
                        SwingPoint(
                            price=bar.low,
    ...
    def _detect_ltf_confirm(
        ...
        for f in fvgs:
            ...
            if fvg_ts == 0:
                logger.debug(
                    "[LTF] %s FVG ts bulunamadı (idx=%s) — temporal filter devre dışı",
                    symbol,
                    f.real_index,
                )
    ...
    def analyze(
        ...
        try:
            ...
            if use_15m_fvg:
                ...
                fvgs_eff = sorted(
                    fvgs_eff, key=lambda f: abs(f.top - f.bottom), reverse=True
                )
                ...
                fvg_timestamp_map = {
                    f.real_index: bar_timestamps_15m.get(f.real_index, 0)
                    for f in fvgs_eff
                }
                    ...
                    for f in fvgs_eff:
                        ...
                        for g in fvgs_2h:
                            if g.direction != f.direction:
                                continue
                            ov = _interval_overlap_ratio(
                                f.bottom, f.top, g.bottom, g.top
                            )
                            if ov >= overlap_min:
                                ok = True
                                break
                ...
                fvgs_eff.sort(
                    key=lambda f: (
                        not validated_map.get(f.real_index, False),
                        -abs(f.top - f.bottom),
                    )
                )
                ...
                bar_timestamps_h1 = (
                    {b.index: b.timestamp for b in bars_h1} if bars_h1 else {}
                )

                # [FIX-7] FVG timestamp map — _detect_ltf_confirm için
                fvg_timestamp_map = {
                    f.real_index: bar_timestamps_h1.get(f.real_index, 0)
                    for f in fvgs_eff
                }
            ...
            events.extend(
                self._detect_ltf_confirm(
                    self.symbol, fvgs_eff, bars_m1, current_close, fvg_timestamp_map
                )
            )
