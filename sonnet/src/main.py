...
class LiveTradingBot:
    ...
    async def _on_1m_close(self, symbol: str, bars_m1: list[Bar]):
        try:
            ...
            # ── STATE-DEBUG: her 1m callback'te basılır (events olsa da olmasa da) ──
            current_state = self.state_machine.get(symbol)
    ...
    async def run(self):
        ...
        try:
            ...
            if listen_key:
                ...
                log.info("[USER_DATA] Listen key oluÅŸturuldu: %s...", listen_key[:10])
                ...
                async def on_account_update(msg: dict):
                    ...
                    if positions:
                        log.debug(
                            "[USER_DATA] ACCOUNT_UPDATE | reason=%s | %d pozisyon güncellendi",
                            reason,
                            len(positions),
                        )
        except Exception as e:
