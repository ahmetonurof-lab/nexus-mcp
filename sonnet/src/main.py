...
class LiveTradingBot:
    def __init__(self):
        ...
        self._last_state_debug: float = 0.0  # STATE-DEBUG periyodik zamanlayıcı (60 sn)
    ...
    async def _load_existing_positions(self):
        ...
        try:
            ...
            for pos in positions:
                ...
                log.info(
                    "[RECOVER] %s pozisyon=%s giriÅŸ=%.4f SL=%d TP=%d",
                    symbol,
                    direction,
                    entry,
                    n_sl,
    ...
    async def _sync_positions(self, current_bar: Bar):
        ...
        try:
            ...
            # PM guard: pozisyon listesi boÅŸsa trade'leri KAPATMA
            if not positions:
            ...
            if symbol not in self.active_trades:
                ...
                if current_state.state == SetupState.WAIT_CONFIRM:
                    try:
                        ...
                        log.error("[PARTIAL] %s send_order hatasi: %s", symbol, e)

            # STATE-DEBUG: her 60 sn'de bir durum raporu
            now_ts = time.time()
            if now_ts - self._last_state_debug >= 60.0:
                self._last_state_debug = now_ts
                current_state = self.state_machine.get(symbol)
                log.info(
                    "[STATE-DEBUG] %s | state=%s | sweep=%s | mss=%s | retrace=%s | ltf=%s | fvg_missed=%s | sweep_tf=%s",
                    symbol,
                    current_state.state,
                    ...
                    current_state.fvg_missed,
                    getattr(current_state, "sweep_tf", ""),
                )
