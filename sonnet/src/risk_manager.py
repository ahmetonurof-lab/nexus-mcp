...
class RiskManager:
    ...
    def __init__(
        ...
        self._available_margin = (
            available_margin if available_margin is not None else balance
        )
    ...
    def calculate_lot(
        ...
        max_lot = (
            (self._available_margin * self.leverage * self.margin_usage) / entry
            if entry > 0
            else 0.0
        )
    ...
    def trailing_sl(
        ...
        if direction == "long":
            new_sl = max(
                current_sl, current_sl + (current_price - current_sl) * step_ratio
            )
        else:
            new_sl = min(
                current_sl, current_sl - (current_sl - current_price) * step_ratio
            )
    ...
    def build_trade(
        ...
        if entry_price is not None:
            ...
            log.debug(
                "[BUILD] %s entry_price yok → FVG midpoint fallback: %.5f", sym, entry
            )

        log.info(
            "[BUILD-IN] %s | entry=%s | h4_sl=%s | h1_tp=%s",
            sym,
            entry,
            h4_swing_level,
            h1_liquidity_level,
        ...
        if h4_swing_level is not None:
            ...
            sl = self.calculate_sl_htf(
                sym, state.direction, entry, h4_swing_level, sweep_level=sweep_lvl
            )
        else:
            ...
            buf = tier["sl_buffer"]
            raw_sl = (
                fvg.bottom * (1.0 - buf) if dire == "long" else fvg.top * (1.0 + buf)
            )
            dist = abs(entry - raw_sl)
        ...
        # ── TP ──
        tp = self.calculate_tp_htf(
            sym, entry, risk_dist, h1_liquidity_level, state.direction
        )
        ...
        try:
            ...
            self.risk_pct = (
                _original_risk_pct  # exception path'te bile restore garantisi
            )
        ...
        net_rr = round(
            (reward_dist - entry * cost_pct) / (risk_dist + entry * cost_pct), 4
        )
