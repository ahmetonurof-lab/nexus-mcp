...
## ✅ Done
- **Dinamik R-aktivasyonlu ATR-Chase taraması (2026-08-07, commit bekliyor):** Sabit % kâr eşiği (coin/bucket bazlı `risk-pts` değişken olduğu için yanlış kalibre) → dinamik R-katı eşiği. `analyzer_v5.py`: `TRAIL_ACTIVATION_PCT` → `TRAIL_ACTIVATION_R_MULT=1.0`; eşik = `R * |entry - initial_sl|` (risk_pts), unrealized kâr puan bazlı; eşik altında `cur_price=None` → ATR-chase fallback pasif. `replay_trailing_v2.py`: `--act-pct` → `--act-r`; bars slotu R değeri taşır; MaxDD sütunu (trade-bazlı kümülatif PnL) önceki session'da eklenmişti, korunuyor. Grid (PYTHUSDT+SEIUSDT, 13 koşu, 739s): R ∈ {0.8, 1.0, 1.2, 1.5} × K ∈ {1.0, 1.5, 2.0}.
  - **Sonuçlar:** A baseline: 7955 trade, TP=902, PE 63.6%, +438,205, MaxDD 1,088, HOP 8154. En iyi D: K=2.0/R=1.5 → +437,071 (A'dan -0.26%), MaxDD 1,035 (-4.9%). K=2.0 tüm R'lerde MaxDD 1,035; K=1.0 tüm R'lerde NetPnL -5.3K ila -7.2K; düşük R (0.8) fallback'i erken aktifleştirip TP'leri kesiyor (A 902 → K=1.0/R=0.8 203 TP, -31,004). K-R etkileşimi: R=1.5/K=1.0 MaxDD 1,220 (en kötü).
  - **Karar:** Hiçbir D kombinasyonu NetPnL'de A'yı geçmiyor; K=2.0 MaxDD iyileşmesi (53 USD) 2-coin gridinde marjinal. **Canlıya değişiklik önerilmedi — A (retrace) sabit.**
