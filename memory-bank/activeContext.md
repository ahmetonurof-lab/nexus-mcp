...
## Current State
2026-08-06: **STATE-SYNC fix (P2-4) canlıda.** Contabo (root@169.58.41.73) üzerinde bot `screen -S bot` içinde `python3 bot.py` (cwd `/root/sniper/src`) çalışıyor; kod `4033198`'e güncellendi (bc73b5c→4033198). Deploy 3 katmanlı teyit edildi: hash ✓, debug grep 0 ✓, davranışsal (10 pozisyon envantere alındı, WS bağlı) ✓. İlk trailing olayında `runtime.protection.sl_current/tp_current` güncellemesi gözlemlenecek — tüm pozisyonlar şu an `TRAIL: 0x`.

## Recently Completed
- **STATE-SYNC fix deploy (2026-08-06, commit `4033198`):** trailing `_replace_one` sonrası flat order ID + runtime.protection atomik senkron fix'i canlıya alındı (bkz. progress.md). Oracle (161.33.184.243) boştu — bot Contabo'da.
