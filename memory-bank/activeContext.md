...
## Recently Completed
- **Log seviyesi düzeltmesi (2026-08-05, baş mühendis onaylı sıra madde 1):** `[P1-15_DEBUG]` (`src/bot.py:582` check_exit öncesi) ve `[POST_ENTRY_DEBUG]` (`src/trading/order_manager.py:385` get_open_order_ids) logları `WARNING` → `DEBUG` çekildi. `trail_skipped` bir log değil; `paper_trade_logger.py` JSONL telemetri event'i — log seviyesi kapsamına girmedi, dokunulmadı.
- **Güncel Öncelik Sırası (2026-08-05, baş mühendis onaylı):** (1) Log seviyesi ✅ YAPILDI, (2) Stale event kök neden araştırması, (3) ATR-chase trailing (K=0.5/1.0/1.5 replay doğrulamalı), (4) FVG gevşetme — "backtest bekliyor" kovasında, backtest kanıtı gelmeden sırada değil.
...
## Next Actions
0. **Güncel öncelik sırası (2026-08-05, baş mühendis onaylı):** (1) Log seviyesi ✅ yapıldı. (2) **Stale event kök neden araştırması** — WS FILLED gecikmesi 87–353 sn aralığının kaynağı (Binance push mu, bot event loop tıkanması mı); ölçümle başla (emir zamanı vs WS FILLED delta serisi), dashboard/alert değil. (3) **ATR-chase trailing** — K=0.5/1.0/1.5 replay doğrulamalı (zaten planlı, K optimizasyonu). (4) **FVG gevşetme** — SIRADA DEĞİL: "backtest bekliyor" kovasında; %87.8 red oranı başlı başına kanıt değil, gevşetilirse hangi trade'ler kabul edilir + kaçı kârlı çıkar somut backtest kanıtı gelmeden sıraya alınmayacak.
